...
LGOPT* CreateAuthBlock(UCHAR LogonType, BOOLEAN bElevated, PCWSTR pcwz, PULONG pcb)
{
	ULONG cb = 0, cch, len, cbNeed = 1;
	PCWSTR pwz = pcwz;
	LGOPT* plo = 0;
	do 
	{
		*pcb = cb + sizeof(LGOPT);
		...
		PSTR psz = plo ? plo->buf : 0;

		while ( cch = (ULONG)wcslen(pcwz))
		{
			if ( len = WideCharToMultiByte(CP_UTF8, 0, pcwz, ++cch, psz, cb, 0, 0))
			{
				if (psz)
				{
					psz += len, cb -= len;
		...
		{
			*psz = 0;
			plo->bElevated = bElevated;
			plo->LogonType = LogonType;
			return plo;
		}

		pcwz = pwz;

	} while (plo = new(cb = cbNeed) LGOPT);
					   ...
					   PULONG pcb)
{
	...
	do 
	{
		len += GetWindowTextLengthW(GetDlgItem(hwnd, id[--i])) + 1;
	} while (i);

	PWSTR psz = (PWSTR)alloca(len*sizeof(WCHAR)), pwz = psz;

	do 
	{
		s = GetDlgItemTextW(hwnd, id[--n], psz, len) + 1;
		psz += s, len -= s;
	} while (n);

	*psz = 0;

	return CreateAuthBlock(LogonType, bElevated, pwz, pcb);
}
